...
- ✅ **AI価格提案・査定**: 商品名・カテゴリ・状態から推奨価格を OpenAI API で算出
- ✅ **📷 カメラ写真AI査定**: 出品時に写真を撮るだけで Gemini Vision が商品を識別 → Google Search grounding でメルカリ・ヤフオク等の相場をリアルタイム検索 → 使用状態を加味した推奨価格・説明文を自動入力
- ✅ **AIエージェント・オートネゴシエーション**: 購入者AI ⇔ 出品者AI が自律会話し、1ターンで価格交渉を完結
- ✅ **AI物々交換マッチング**: 3人以上ユーザー間の需要循環（わらしべ長者ループ）を自動検出・差額清算
- ✅ **AI使用風景生成**: 商品画像＋プロフィール写真を Gemini 2.0 Flash Exp で合成し、使用情景を自動生成
- ✅ **AI出品審査・モデレーション**: 禁止物品・不適切コンテンツを OpenAI Moderation API で自動検知
- ✅ **マルチAI対応**: OpenAI（GPT / Moderation）+ Gemini（Vision / Search grounding / 画像生成）の用途別使い分け設計
...
| **AI API** | OpenAI (GPT, Moderation) + Gemini (Vision / Search / 画像生成) | 用途別使い分け・マルチAI冗長設計 |
...
3. **画像処理**: Distroless コンテナでの MIME type 問題は対応済み。AI使用風景生成は OpenAI `/v1/images/edits` の mask 形式誤用を解消し Gemini 2.0 Flash Exp に切り替え済み
...
- **UI画面**: 8 メイン + 2 モーダル
- **AI連携**: 8 機能（説明生成・Q&A・価格提案・**カメラ写真査定**・ネゴシエーション・物々交換・シーン生成・モデレーション）
...
### 4. 📷 カメラ写真AI査定（Photo Appraise）
出品画面でカメラボタンを押すだけで、撮影した商品を Gemini Vision が瞬時に識別。さらに Google Search grounding（Gemini 2.0 Flash 内蔵）が日本のフリマサイトの相場をリアルタイム検索し、商品の使用状態を加味した推奨価格・最低価格・説明文を自動入力します。
*   **2ステップAI処理**: Step1（画像→商品識別・JSON出力）→ Step2（Google Search grounding→リアル相場取得）
*   **ワンクリック反映**: 「この情報を出品フォームに反映する」で商品名・カテゴリー・価格・説明をまとめて自動入力
*   **モバイル完全対応**: `capture="environment"` による端末カメラ起動（PC ではファイル選択）

### 5. 📸 映画風シネマグラフ & Gemini画像合成（AI Scene Synthesis）
*   Gemini 2.0 Flash Exp と連動した、商品の「実際の部屋や屋外での使用風景イメージ写真」を自動で合成するシステム。商品画像＋ユーザーのプロフィール写真を入力画像として渡し、AIが自然な使用シーンを生成します。
